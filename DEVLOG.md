...
**Deferred (see TODO — reasons recorded).** Per-session unread badge + live ASSISTANT injection into an open thread (history-on-open + notification cover the core; unread-by-session wants `chat_id`→`session_id` mapping); retire `HermesInboxScreen` + deep-link re-point (needs a select-session-on-entry signal); outbound reply outbox/retry (needs multiplexer connection-state); **agent-initiated** named Threads (the upstream `send_message` thread/chat_id param — user-initiated create now ships).

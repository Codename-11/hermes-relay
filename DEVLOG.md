...
- **Verification.** `:app:testGooglePlayDebugUnitTest --tests GatewayChatClientTest` green; `lintGooglePlayDebug` green; full `assembleGooglePlayDebug` green; network-resilience verified e2e on-device. Image rendering compiles + lints; on-device visual check PENDING (device idle-locked mid-run). Model-switch UI (#3) NOT done — needs device unlock for UI/server iteration.

...
- **Relay realtime-agent loop drift (correctness).** The non-native ("render-after-Hermes") websocket loop in `plugin/relay/realtime_agent/broker.py` lacked a `playback.drained` branch, so the end-of-turn ack every client sends fell through to the unsupported-message error; the client treats `voice.error` as fatal and tore the session down on every turn whenever a non-native provider was configured. Extracted the client→server messages that are identical across the native and non-native loops (`session.start`, `session.resume`, `client.ack`, `playback.drained`, `hermes.confirm`) into a shared `_handle_common_client_message` dispatcher used by both loops so they can no longer drift, and added the missing `input_audio.clear` handling to the non-native path. The native loop's per-message `provider_task.done()` break check is preserved exactly. (The `hermes.confirm` echo is by-design in both loops — the realtime model answers confirmations via its `hermes_confirm` tool, which itself returns `forwarded_to_hermes_ui` — so it was left unchanged.)
...
- **Provider-aware enhanced voice (Gemini + xAI) — relay path.** `/voice/synthesize` accepts optional per-request enhanced overrides (`voice`, `model`, `audio_tags`, `persona_prompt`, `language`) mapped onto the active provider. Upstream's `text_to_speech_tool` has no per-call override surface, so the relay merges the overrides into a config copy and invokes the provider generator directly — `_generate_gemini_tts` for Gemini (voice/model/`audio_tags` tone-tag rewrite/inline persona via a temp file) and `_generate_xai_tts` for xAI (`voice`→`voice_id`, `audio_tags`→`auto_speech_tags`, `language`). Gemini's audio-tag rewrite fails soft when the auxiliary LLM is unavailable. `/voice/config` advertises a provider-aware `tts.enhanced` capability block (voices/models + `supports_persona`/`supports_language`/`audio_tags_label` flags) when the basic provider is Gemini or xAI. Android plumbs `EnhancedVoiceOverrides` from new persisted preferences through `RelayVoiceClient.synthesize` and the relay adapter, and an "Enhanced Voice (<provider>)" section in Voice Settings renders from the capability flags: curated voice dropdown for Gemini / free-text for xAI, model picker, tone/speech-tags switch (gated to a Gemini 3.1 model where required), plus persona (Gemini) and language (xAI) fields. OpenAI is intentionally excluded — upstream exposes only voice/speed for it (no `instructions` tone steering).
- **Enhanced voice on the streaming renderer (`/voice/output`).** Because `voice_output_enabled` defaults true, the streaming renderer — not `/voice/synthesize` — is the normal relay playback path, so the per-request override above was effectively fallback-only. Added xAI `auto_speech_tags` as a per-profile `voice_output:` setting threaded through every layer (config dataclass + env + YAML loader, `voice_output_settings`, profile override, `VoiceOutputSession`, `_provider_options`, `config_payload`, the `PATCH /voice/output/config` allow-list), mirroring `text_normalization`. The render applies `upstream_voice.apply_xai_speech_tags()` (upstream's inline/wrapping tone markers, fail-soft) to each chunk before the `voice_lab` `xai_tts` renderer — keeping `voice_lab` standalone and the upstream import in the patch-point. Android: `VoiceOutputConfig.auto_speech_tags` + `updateVoiceOutputConfig(autoSpeechTags=)` + an "Expressive speech tags" switch in the **Hermes Chat + Voice Output** settings card (shown for `xai_tts`, persisted with the existing Save buttons). The `voice_lab` set is xai/openai/elevenlabs — no Gemini streaming provider, so Gemini enhanced voice stays `/voice/synthesize`-only. Tests: PATCH round-trip persists/echoes `auto_speech_tags`; `apply_xai_speech_tags` call-through + fail-soft.
- **Docs.** `docs/upstream-surface-matrix.md` gained a "Voice Surfaces (standard vs. relay)" section with an explicit **route-ownership table** (every `/voice/*` route is relay-owned; only dashboard `/api/audio/*` is upstream — no upstream streaming/WS audio route exists) and an enhanced-voice matrix covering both relay paths; `docs/spec.md` Phase V documents the override, the `tts.enhanced` block, and the `voice_output` `auto_speech_tags` control; `user-docs/features/voice.md` gained an "Enhanced Voice (Gemini & xAI)" section + the streaming speech-tags toggle and corrected the stale `~/voice-memos` cleanup note.
- **Standard voice polish.** Added a pre-flight 25 MB guard (matches upstream `_MAX_TRANSCRIPTION_UPLOAD_BYTES`) and friendly 413/400 copy in `VoiceAudioClient`; hardened the dashboard audio-route HEAD probe to also try `/api/audio/speak` so a reverse proxy that rewrites 405→404 doesn't read as absent.
- **Verification.** Python: `python -m py_compile` on all four touched relay modules; full relay voice suite (`test_voice_routes`, `test_voice_output_routes`, `test_realtime_agent_routes`, `test_realtime_voice_routes`, `test_profile_voice_config`) green at 100 tests except one pre-existing xAI-OAuth env-dependent failure (confirmed identical on the unmodified tree). New tests: a non-native `playback.drained` regression (verified red on the pre-fix loop, green after), a Gemini-override synthesize integration test, and pure-function coverage for the override parser + capability block. Kotlin changes are not built locally (Studio + `./gradlew lint` are the pre-push gate). Deferred: surfacing the upstream `provider` field into voice stats, a `test_upstream_voice.py` adapter-drift guard, and an upstream `prompt.submit` ephemeral-field PR for gateway voice turns.

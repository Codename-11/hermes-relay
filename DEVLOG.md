...
## 2026-06-22 — Released plugin-v1.2.1 + live-server deploy

Cut the Plugin 1.2.1 release — a Realtime Agent reliability patch — and deployed it to the live server. Both fixes were already on `dev`: the `session_not_found` brokered-handoff fix (`f6b965a`) and the realtime voice heartbeat-during-long-runs fix (`d1820fb`); 1.2.1 only adds the version bump and release packaging. Release-prep bumped the six plugin version sources via `scripts/bump-plugin-version.sh` (sync check green), folded the relay `session_not_found` fix into the existing `[1.2.1]` `CHANGELOG.md` line (the Desktop-CLI entries stay under `[Unreleased]` for their own `cli-v*` cut), and rewrote `PLUGIN_RELEASE_NOTES.md` as a Fixed-only release body. The pre-existing local `TODO.md` edit was kept out via pathspec commit.

- **Release.** `dev` had drifted behind `main` (12 Dependabot bumps merged straight to `main` + 3 prior `dev`→`main` release-merge commits never back-merged), so the release PR #119 was `BEHIND`; `gh pr update-branch` merged `main` into `dev` (conflict-free — no overlap with the version/CHANGELOG files). Only `Required checks` + `claude-review` gate `main` (the path-optimized sentinel pattern); both green, with the plugin-relevant jobs (focused plugin tests, dashboard build, Python syntax) also green on the head. Merged `--no-ff` (merge `41037a3`); `plugin-v1.2.1` tagged from the `main` tip triggered `release-plugin.yml` → validate-metadata → wheel + sdist + `SHA256SUMS.txt` → GitHub Release **Hermes-Relay-Plugin v1.2.1**.
- **Live deploy.** `docker-server:~/.hermes/hermes-relay` was at `d1820fb` (had the heartbeat fix but **not** `f6b965a`) reporting `/health` `1.2.0`. Fast-forwarded to `788d2ab` (`--ff-only`, leaving the on-box doc/SKILL edits untouched — none overlapped the upstream delta) and restarted `hermes-relay` (active, clean). `/health` now reports `1.2.1` with both fixes live. Session state survived the restart (3 sessions before and after — relay sessions now persist, so no forced re-pair).

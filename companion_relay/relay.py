"""Companion Relay — Main WSS server.
...
Usage:
    python -m companion_relay
    python -m companion_relay --port 8767 --no-ssl
    python -m companion_relay --config /path/to/config.yaml
"""
...
logger = logging.getLogger("companion_relay")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Hermes Companion Relay — WSS server for the Android companion app"
    )
    parser.add_argument(
        "--host",
        default=None,
    ...
    logger.info(
        "Starting Companion Relay v%s on %s://%s:%d",
        __version__,
        scheme,
        config.host,
        config.port,

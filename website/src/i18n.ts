...
export const translations: Record<Locale, Copy> = {
  en: {
    ...
    device: {
      ...
      lede: 'Hermes-Relay prefers upstream for standard Chat, Manage, Voice, and inbound files. The plugin fills current gaps with Terminal and TUI, notifications, desktop tools, enhanced voice, Relay sessions, optional Device Control, and media compatibility or metadata. When upstream owns a compatible capability, Relay defers to it.',
      capabilities: 'Relay plugin capabilities', read: 'Read the screen', tap: 'Tap and scroll', type: 'Type for you',
      terminal: 'Terminal + TUI', notifications: 'Notifications', media: 'Media enhancements', desktopTools: 'Desktop tools', enhancedVoice: 'Enhanced voice', deviceControl: 'Device Control',
  ...
  de: {
    ...
    device: {
      ...
      lede: 'Hermes-Relay bevorzugt Upstream für Standard-Chat, Manage, Voice und eingehende Dateien. Das Plugin schließt aktuelle Lücken mit Terminal und TUI, Benachrichtigungen, Desktop-Werkzeugen, erweiterter Voice, Relay-Sitzungen, optionalem Device Control sowie Medienkompatibilität oder Metadaten. Sobald Upstream eine kompatible Funktion besitzt, hat diese Vorrang.',
      capabilities: 'Funktionen des Relay-Plugins', read: 'Bildschirm lesen', tap: 'Tippen und scrollen', type: 'Für dich schreiben',
      terminal: 'Terminal + TUI', notifications: 'Benachrichtigungen', media: 'Medienerweiterungen', desktopTools: 'Desktop-Werkzeuge', enhancedVoice: 'Erweiterte Voice', deviceControl: 'Device Control',
  ...
  es: {
    ...
    device: {
      ...
      lede: 'Hermes-Relay prioriza upstream para Chat, Manage, voz estándar y archivos entrantes. El plugin cubre las carencias actuales con Terminal y TUI, notificaciones, herramientas de escritorio, voz mejorada, sesiones Relay, Device Control opcional y compatibilidad o metadatos multimedia. Cuando upstream ofrece una función compatible, Relay le cede el paso.',
      capabilities: 'Funciones del plugin Relay', read: 'Leer la pantalla', tap: 'Tocar y desplazar', type: 'Escribir por ti',
      terminal: 'Terminal + TUI', notifications: 'Notificaciones', media: 'Mejoras multimedia', desktopTools: 'Herramientas de escritorio', enhancedVoice: 'Voz mejorada', deviceControl: 'Device Control',
  ...
  ja: {
    ...
    device: {
      ...
      lede: 'Hermes-Relay は標準 Chat、Manage、Voice、受信ファイルで upstream を優先します。plugin は現在の不足を Terminal と TUI、通知、デスクトップツール、拡張 Voice、Relay セッション、任意の Device Control、メディア互換性やメタデータで補います。upstream に互換機能がある場合はそちらを優先します。',
      capabilities: 'Relay plugin の機能', read: '画面を読み取る', tap: 'タップとスクロール', type: '代わりに入力',
      terminal: 'Terminal + TUI', notifications: '通知', media: 'メディア拡張', desktopTools: 'デスクトップツール', enhancedVoice: '拡張 Voice', deviceControl: 'Device Control',
  ...
  'pt-BR': {
    ...
    device: {
      ...
      lede: 'O Hermes-Relay prioriza upstream para Chat, Manage, voz padrão e arquivos recebidos. O plugin cobre lacunas atuais com Terminal e TUI, notificações, ferramentas de desktop, voz avançada, sessões Relay, Device Control opcional e compatibilidade ou metadados de mídia. Quando upstream oferece uma função compatível, o Relay cede a ela.',
      capabilities: 'Recursos do plugin Relay', read: 'Ler a tela', tap: 'Tocar e rolar', type: 'Digitar por você',
      terminal: 'Terminal + TUI', notifications: 'Notificações', media: 'Aprimoramentos de mídia', desktopTools: 'Ferramentas de desktop', enhancedVoice: 'Voz avançada', deviceControl: 'Device Control',
  ...
  'zh-CN': {
    ...
    device: {
      ...
      lede: 'Hermes-Relay 在标准 Chat、Manage、Voice 和入站文件上优先采用 upstream。plugin 用 Terminal 与 TUI、通知、桌面工具、增强 Voice、Relay 会话、可选 Device Control，以及媒体兼容性或元数据补足当前缺口。当 upstream 提供兼容功能时，Relay 会优先采用 upstream。',
      capabilities: 'Relay plugin 功能', read: '读取屏幕', tap: '点击和滚动', type: '代您输入',
      terminal: 'Terminal + TUI', notifications: '通知', media: '媒体增强', desktopTools: '桌面工具', enhancedVoice: '增强 Voice', deviceControl: 'Device Control',

...
**Active — experimental desktop computer-use MVP:** Plan at [`docs/plans/desktop-computer-use-mvp.md`](docs/plans/desktop-computer-use-mvp.md). Current slice extends the existing desktop tool channel with opt-in `desktop_computer_*` contracts for status, screenshot observe mode, grant request/cancel, and Windows-only action execution behind durable computer-use consent, task-scoped grants, and local per-action CLI approval. It does not implement unrestricted or silent mouse/keyboard automation and does not depend on npm publication.

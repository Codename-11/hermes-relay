...
**Active — experimental desktop computer-use MVP:** Plan at [`docs/plans/desktop-computer-use-mvp.md`](docs/plans/desktop-computer-use-mvp.md). First slice extends the existing desktop tool channel with opt-in `desktop_computer_*` contracts for status, screenshot observe mode, grant request/cancel, and fail-closed action requests. It does not implement unrestricted mouse/keyboard automation and does not depend on npm publication.

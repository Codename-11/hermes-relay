...
- **Phase 2c — two-way reply (the missing half; surfaced in live testing).** Proactive is currently **push-only**: `ProactiveMessageNotifier` builds a plain notification with **no reply action** (no `RemoteInput`), and the relay `ProactiveChannel` is **one-way** (no phone→server reply envelope; docstring says fire-and-forget). So the agent can reach you but you can't answer. Closing it: (1) an inline-reply notification (`RemoteInput` + a reply BroadcastReceiver), (2) a `proactive.reply` envelope (app→relay) + a relay→agent path that posts the reply back into the originating conversation (keyed by `chat_id`/`reply_to` carried on `phone.message`), and (3) a reply box in the Hermes inbox. This is what turns it from a one-way ping into a conversation.

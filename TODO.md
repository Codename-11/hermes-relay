...
## Active — 1.4.1 Chat + Voice enhancement batch (2026-07-09)

Implementation plan: `docs/plans/2026-07-09-1.4.1-chat-voice-enhancements.md`.
This batch is staged on a feature branch from `dev` after the background-route-loss
fix. The Chat and Voice implementation waves are integrated locally. Version bumps,
release artifacts, merge, push, tags, deployment, and on-device release sign-off
remain separate owner-controlled steps.

---

## Active — 1.4.1 release verification

The obsolete 1.4.0 release hold is superseded by this 1.4.1 batch. Before release
preparation, keep these owner/device gates explicit:

- Repeat the exact record → background/route loss → foreground reproduction on the
  newly deployed relay and APK; no `Listening...` / `Still working...` row may strand.
...
- Run release preparation separately: 1.4.1 versioning and public release artifacts,
  then owner-controlled merge/tag/deploy/upload. Nothing in this batch authorizes it.

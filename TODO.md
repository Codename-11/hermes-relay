...
- `**markdown-renderer` pinned at `0.41.0` (compileSdk ceiling)** — `MarkdownContent.kt` is now on the 0.4x API (`MarkdownHighlightedCodeBlock`/`Fence`, m3 `markdownColor`, `Highlights.Builder`), so the old 0.30.0 API blocker is resolved. The current ceiling is **build-tooling, not API**: `0.42.0` (Dependabot #117, `f8141a6`, merged to `dev` 2026-06-22 after the 1.2.1 release) requires `compileSdk 37`, but we target `compileSdk 36` (only `android-36`/`36.1` SDK platforms installed) — it broke the `dev` build and was reverted to `0.41.0` during 1.2.2 prep. **Do not bump `markdown-renderer` past 0.41.0 without also bumping `compileSdk` to 37** (installing the platform + clearing CLAUDE.md's "Compile SDK 36"). Add a Dependabot ignore rule for `markdown-renderer` to stop the recurring break (see next item).

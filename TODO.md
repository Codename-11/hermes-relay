...
- **Pet behavior model — richer state association (spec'd 2026-06-19, `docs/pet-spec.md` "Agent states & pet behavior").** Honesty clamp shipped: declared `tools`/`intensity` are now intersected with `PET_RENDERER_CAPABILITIES` so the badge can't over-promise; a friendly `writing` clip alias for the Streaming state shipped. Remaining, grounded in the prior-art research (MS Agent `Think`≠`Process`; `pi-animations` Thinking·Working·Tool; Peon Pet one-shots):
  - **`working`/tool-use clip + state.** Add a distinct "agent running a tool" behavior, separate from `thinking`. Either a 7th `SphereState` or a pet-local sub-state derived from the already-plumbed `toolCallBurst` in `PetAvatar.Render` (cheaper, no Sphere/call-site blast radius); chain `working`→`writing`→`thinking`→`idle`. Flip `PET_RENDERER_CAPABILITIES.tools` once wired.
  - **One-shot reaction overlays.** `greet`/`wake` on connect, `celebrate`/`done` on turn completion, `attention` on notification — play once over the base loop, then return. Needs a one-shot layer in `PetAvatar.Render` (base loop + transient overlay that auto-expires).
  - **Continuous modulation.** Consume `intensity` (stream rate / activity) for idle+working liveliness; un-clamp `PET_RENDERER_CAPABILITIES.intensity` when done. Mirror how `MorphingSphere` already consumes these.

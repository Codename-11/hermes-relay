...
- **Threads surface (unified-session model — see ADR 12 + the Refinement above).** Build order, each shippable: **(1)** source tags in the session drawer (`source=phone` → clean **Threads** chip + thread-spool icon, NOT a phone glyph) — also delivers the "source attribution in Chat" goal; **(2)** open a Thread in Chat from its session-store history (reuse the existing message-history path); **(3)** route the live `proactive` push into the session view + notification + unread, demoting `ProactiveInboxStore` to cache/outbox; **(4)** reply from the Chat composer via `proactive.reply` + persist the user turn + local `Sending/Queued/Failed` status — **MVP**; **(5)** a **Threads capability row** in the best-path UI + a pinned **Threads** entry atop the drawer (thread-spool icon, shown only when relay-paired + opted-in) + retire `HermesInboxScreen`, re-point the notification deep-link + Settings "View messages"; **(6)** outbox/retry on reconnect; **(7)** relay `proactive.reply.ack` (honest Delivered) + `proactive.cancel`; **(8)** multi-thread `chat_id` (named/project Threads). **Verify gate before (1):** confirm the app's session-list/history path surfaces a `source=phone` session cleanly (upstream `session.list` returns all sources flat, so it should — but check whether the drawer currently filters it out). Honesty call: do NOT show "Delivered" until (7) lands (can't confirm it client-side before the ack).
  - **Status (2026-06-29, implemented UNBUILT — verify in Studio):** **CODE-COMPLETE on `dev`:** slice **1** (drawer source tags + `ThreadSpoolGlyph` + Threads filter), **2** (open a Thread from history — free via the existing `loadSessionHistory` path), **3-parse** (carry `reply_to` on `ProactiveMessage`), **4** (composer reply in a `source=phone` session routes over `proactive.reply`; `MessageDeliveryStatus` SENDING→DELIVERED/FAILED on the bubble), **5** (Threads capability row in `SessionPathCard` + `threadsCapabilityActive` drawer wiring), **7** (relay `proactive.reply.ack` + `proactive.cancel` — 25/25 `unittest` green — and client ack handling). **DEFERRED (reasons):** 3-live-append + per-session **unread badge** (history-on-open + the notification already cover the core; unread-by-session wants a `chat_id`→`session_id` map that's cleaner after multi-thread; live ASSISTANT injection into the exact thread is the riskiest untested edit); 5-**retire `HermesInboxScreen`** + deep-link/`onOpenInbox` re-point to open the Thread in Chat (needs a select-session-on-entry signal — safer with device feedback; the inbox still works via Settings meanwhile); 6 **outbox/retry** (needs multiplexer connection-state + on-device send-failure behavior); 8 **agent-facing `chat_id`** (relay + adapter already generic — only the upstream `send_message` thread/chat_id param + the app naturally listing multiple `source=phone` sessions remain; no relay code).
...
- **(AFTER the gateway/agent surface ships) — show chat *source/platform* attribution in the normal Chat surface, like the official desktop.** Hermes-desktop tags each session with the gateway it came from (Discord / Slack / API / phone / …); our app appears to surface everything generically under "api". Once the phone/agent lane exists as a distinct attributed source, investigate: (a) display a small source chip/label on chat sessions in the drawer/list (where the conversation originated); (b) optional clean **sort by source**; (c) optional **filter/hide other sources** (e.g. focus on just phone-agent, or hide API noise). Must stay clean and unconfusing — a quiet attribution affordance, not a heavy filter UI. First confirm what platform/source field upstream actually exposes on `/api/sessions` (the gateway-attribution finding from the agent-surface design covers this) and whether the api_server SSE sessions even carry a source. Standard-path rule: read-only display of an upstream-provided field; don't require a server change for the basic Chat tab.

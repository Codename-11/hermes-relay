...
- **Phase 2c reply round-trip (off-device).** With a message delivered, reply both from the notification inline-reply and the Hermes inbox reply box; confirm the agent receives it on the `phone` platform and its answer comes back to the phone in the *same* thread (not a new conversation). Confirm the gateway adapter's poll loop reaches the relay's loopback `GET /phone/replies` (different process), that `role_authorized=True` lets the reply through without `PHONE_ALLOW_ALL_USERS`, and that a reply typed while disconnected is dropped gracefully (no crash). Sanity-check the notification confirmation state (spinner cleared → "You: …").

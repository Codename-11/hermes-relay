...
- **Realtime voice: make provider-answered turn promotion durable, not only next-turn (2026-07-08).** Provider-answered realtime turns (those the OpenAI/xAI session answers directly, `ChatMessage.realtimeTurn != null`) are only promoted into the durable Hermes session *lazily* — `RealtimeTurnSyncBuilder.buildSyntheticMessages()` folds them in as provenance-tagged synthetic user/assistant pairs on the **next** Hermes-backed turn (`ChatViewModel.kt:4039–4050`; guard flipped by `markRealtimeTurnsSynced()` at `:4310`). The app has no local message persistence (history is server-backed via `loadMessageHistory`), so any unflushed tail is lost when a voice session ends without a follow-up Hermes turn, on app restart, or when the socket dies — including the xAI 900s idle drop above. Two coverage gaps: (a) no **proactive** flush on voice-mode exit or socket-drop — promotion only rides the next chat/run request; (b) the sync is guarded to `streamingEndpoint != "gateway"` (`ChatViewModel.kt:4307`), so on the `/api/ws` gateway transport provider turns may not sync at all (same class as `voice-turn-ephemeral-context-and-gateway-gap`). Fix direction: flush pending `RealtimeTurnTrace`s proactively (voice-exit + socket-drop) and cover the gateway path, so provider-answered turns survive without depending on a later Hermes turn. **Deliberately NOT a separate relay transcript store** — the existing design folds these into the one Hermes conversation on purpose (keeps "what did you just say?" working across the voice/chat boundary); a parallel store would fork the conversation into two histories.
- **Realtime voice: subtle "Voice" provenance chip for provider-answered turns (2026-07-08).** Provider-answered voice turns already carry provenance (the `[Realtime Agent provider-native voice turn: provider=…]` annotation baked into the synced text by `RealtimeTurnSyncBuilder`, plus the live `applyRealtimeAgentEvent` badges). Surface it visually with a small, quiet chip reusing the `SessionSourceBadge`/`SourceChip` precedent (`ui/components/SessionSourceBadge.kt`) — Hermes-backed turns and normal chats stay unbadged/standard; only the provider-answered ones get a low-key "Voice" tag. Keep it as understated as the existing source chips, not a loud treatment. Pairs with the durability item above.

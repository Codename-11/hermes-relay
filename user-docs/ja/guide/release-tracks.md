...
| Chat、プロファイル、Manage、Voice、受信ファイル | はい | はい |
| Relay のターミナル、メディア拡張、通知 | はい | はい |
...
オプションの **Relay プラグイン**は、ターミナル、通知、デバイスチャンネルを
Hermes ホストへ接続し、メディア互換性とメタデータを追加します。通常の受信
ファイルは標準 Dashboard を使用します。

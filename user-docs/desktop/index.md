...
Access is scoped to the selected host. **Ask** keeps the daemon connected without attaching desktop tools. **Structured** enables typed operations but withholds terminal, PowerShell, detached-process, and command-job launch. **Trusted** adds those raw execution tools while screen and input remain task-granted. **Full Access** removes those task prompts for that host; authentication, audit, client revocation, emergency stop, and Windows UAC boundaries still apply. Raw USB has its own per-host Off/Ask/Allow gate for direct USB utilities and secondary services such as ADB. Approval requests appear directly as focused local cards without requiring the main popup or a separate terminal.

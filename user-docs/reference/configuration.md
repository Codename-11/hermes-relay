...
| Dashboard URL | DataStore | The one Dashboard/Gateway address entered during normal onboarding. A second public sign-in URL is not universally required: Android discovers a different provider callback only when upstream advertises one, verifies the session, and asks before saving it. Matching `install_id` values prove the route belongs to the same installation; older gateways require explicit confirmation. Public origins require HTTPS, while same-origin or explicitly reviewed literal LAN/Tailscale/loopback HTTP retains upstream's trusted-network mode. |

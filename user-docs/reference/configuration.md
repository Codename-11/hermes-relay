...
- **Routes (N)** (when Vanilla Hermes setup saved a route or pairing carried multi-endpoint candidates) — the app re-probes when the network changes. Tailscale is the recommended remote route, followed by supported public TLS / Direct Secure Link and local fallbacks. The experimental **Hermes Reach** broker route is shown separately and attempted only after supported routes are unavailable.

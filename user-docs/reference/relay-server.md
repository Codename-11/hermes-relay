...
| `/usage/providers` | GET | Bearer-auth'd provider-neutral account usage for Codex, Nous, and OpenCode Go. Optional `profile=<id>` scopes credentials and account data to that Hermes profile. Disabled by default; set `RELAY_PROVIDER_USAGE_ENABLED=1`. Reuses upstream Hermes account models where available and never returns provider credentials. |

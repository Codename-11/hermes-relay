...
| `app/src/main/kotlin/.../network/handlers/BridgeCommandHandler.kt` | Phase 3 accessibility — routes inbound `bridge.command` envelopes to `ActionExecutor` and emits `bridge.response`. **Full path inventory (post v0.4 wave 1/2/3):** `/ping`, `/setup`, `/events`, `/events/stream` (early-return cluster, work even when service unbound) → service+master+blocklist gate → `/current_app`, `/get_apps` (+ `/apps` legacy alias), `/open_app`, `/tap`, `/tap_text`, `/long_press` (A1), `/type`, `/swipe`, `/drag`, `/scroll`, `/describe_node` (A4), `/press_key`, `/wait`, `/clipboard` (GET+POST, A6), `/media` (A7 play/pause/next/previous/toggle), `/screen` (multi-window via `snapshotAllWindows`, P1), `/find_nodes` (filtered search), `/screen_hash` (A5 cheap change detection), `/diff_screen`, `/screenshot`, `/location`, `/search_contacts`, `/call`, `/send_sms` (last four are sideload-only via `BuildFlavor.isSideload` gate, return 403 on googlePlay), `/send_intent`, `/broadcast` (B4 raw-Intent escape hatches with target-package blocklist). `/tap` + `/scroll` accept optional `nodeId` and resolve via `ScreenReader.findNodeById` against `snapshotAllWindows()` — nodeId wins over (x,y). `/screen` serializes `ScreenContent` via `Json.encodeToJsonElement`. **Phase 3 safety-rails:** optional `safetyManager: BridgeSafetyManager?` constructor arg runs the Tier 5 three-stage check — currentApp blocklist → destructive-verb confirmation (for `/tap_text` + `/type`, suspend-awaits the overlay modal) → reschedule auto-disable timer. `/call` and `/send_sms` ALWAYS confirm regardless of verb match and **fail closed if `safetyManager == null`** (M1 fix). `/open_app`, `/send_intent`, `/broadcast` additionally blocklist-check the **target** package (defense-in-depth). Blocked packages return 403 `{"error": "blocked package <name>"}`, denied confirmations return 403 `{"error": "user denied destructive action", "reason": "confirmation_denied_or_timeout"}`. **Baseline-handler note (2026-04-14):** `/get_apps`, `/apps`, `/open_app`, `/setup` were added by `feature/bridge-feature-expansion` to close a latent v0.3.0 regression — the Python relay registered the routes during Phase 3 Wave 1 but the Kotlin dispatcher silent-dropped them into the unknown-path 404, so `android_get_apps()` and `android_open_app()` had been broken since v0.3.0 shipped. `/get_apps` requires the `<queries><intent><action MAIN/><category LAUNCHER/></intent></queries>` declaration in `app/src/main/AndroidManifest.xml` to enumerate launchable apps on Android 11+ targets. |
...
| `app/src/main/kotlin/.../bridge/BridgeStatusOverlay.kt` | Phase 3 safety-rails — `WindowManager` overlay host. One singleton per process; implements `ConfirmationOverlayHost` so `BridgeSafetyManager` can reach both the destructive-verb modal and the optional floating status chip through the same slot. `TYPE_APPLICATION_OVERLAY` on API 26+, fallback to `TYPE_PHONE` below. Chip is `FLAG_NOT_FOCUSABLE` (taps fall through); modal is `FLAG_DIM_BEHIND` full-screen (intercepts touches). `ComposeView` attachments are wired to a synthetic `OverlayLifecycleOwner` that implements all three tree owners: `LifecycleOwner` (always-RESUMED), `ViewModelStoreOwner`, and **`SavedStateRegistryOwner`** (init sequence: CREATED → `performRestore(null)` → RESUMED — order matters, otherwise `SavedStateRegistryController` asserts). Compose's `AndroidComposeView.onAttachedToWindow` hard-requires all three unconditionally, regardless of whether the composable actually reads saved state — the pre-v0.4 impl skipped `SavedStateRegistryOwner` on the theory "no `rememberSaveable` = no saved state needed" and crashed with `IllegalStateException: Composed into the View which doesn't propagateViewTreeSavedStateRegistryOwner` the first time the user enabled the status chip (Samsung S24 / Android 14 / Compose BOM 2024.12, 2026-04-14). |

...
- **Connection status banner: live, animated, dismissible.** While reconnecting or handing off between routes, the status banner shows a live step list — each step spins while checking, turns a green check on success, or a red ✕ on failure (up to three steps) — and can be dismissed with a **×** or a swipe up. It expands and collapses smoothly above the screen instead of snapping. Returning to the app from the background no longer flashes a misleading "connection changed" for what is the same connection re-handshaking.

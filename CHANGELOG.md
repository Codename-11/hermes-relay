...
- **Sensitive-media classification (relay, opt-in).** A new "Agent context" toggle in the Relay dashboard teaches the agent — server-side, via a removable system-prompt block — to mark private/NSFW media so the phone blurs it per your setting. Off by default; the exact injected instruction is visible in the chat "What the agent sees" sheet under "Relay context (server-side)". No on-device or relay-side classifier — sensitivity stays model-emitted. Removable by uninstalling the relay plugin. See `docs/plans/2026-06-20-relay-enhancement-layer.md`.

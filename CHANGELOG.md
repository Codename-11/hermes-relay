...
- **MorphingSphere on the docs site.** New `SphereMark.vue` component (in `user-docs/.vitepress/theme/components/`) renders a compact 56×26 sphere in a dedicated band between the hero and feature grid. Imports `preview/web/sphere.js` directly so `MorphingSphereCore.kt` remains the single source of truth across app / preview / docs. Two interaction layers: (1) an autonomous low-frequency fbm drift gives the sphere a subtle "looking around" gaze, (2) pointer proximity within 320px pulls the gaze toward the cursor, ramps intensity, and retargets Idle → Listening; leaving retargets back to Idle. Respects `prefers-reduced-motion` (freezes time, zeroes gaze offset) and pauses drawing while scrolled off-screen via `IntersectionObserver`. Wrapped in `<ClientOnly>` so VitePress SSR doesn't touch the canvas.

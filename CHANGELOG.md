...
- **Hot-swap the active Hermes profile from chat.** Picking a server profile in the agent sheet now actually switches the running agent on the gateway — its SOUL, model, and skills change in place via `config.set {key:"profile"}` (the session-scoped mirror of the model switch), matching the official desktop's clean hot-swap with no new session and no lost context. (Previously the gateway ignored the pick — its bare `prompt.submit` carries no profile; SSE turns were already fine via per-request `profileName`.)

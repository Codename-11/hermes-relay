...
- **Connection status now stays out of your way.** Connection feedback is tiered by how much it matters, so a routine reconnect no longer shoves the whole screen down: an in-progress reconnect shows only a small amber "Reconnecting…" cue in the bottom status strip (chat content doesn't move); a recovery or route switch slides in a brief banner that dismisses itself; and only a sustained problem you should act on (no connection, no internet, server unreachable) takes space with the full banner. The banner keeps its live, animated step list (each step spins while checking, turns a green check on success or a red ✕ on failure, up to three steps) and its **×** / swipe-up dismissal. Returning to the app from the background no longer flashes a misleading "connection changed" for what is the same connection re-handshaking.

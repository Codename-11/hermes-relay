...
- **Voice: no more spoken run IDs or phantom "queueing."** The realtime voice model no longer reads 32-character run IDs aloud after starting a background task (identifiers stay out of everything it's asked to speak), no longer claims to "queue" requests (there is no queue), and a completed task's answer is spoken directly — deferral filler like "one moment while I look that up" in place of a finished result now triggers the fallback that speaks the real answer.
...
- **Voice: realtime sessions no longer expire after 15 quiet minutes.** xAI ends a realtime conversation after 900 seconds of inactivity, and nothing flows on the provider socket while you're not talking — so a voice session left open through a long background task (or simply left open) died with a raw provider error. The relay now keeps the provider conversation alive with a periodic sliver of silent audio while the session is quiet (`RELAY_VOICE_PROVIDER_KEEPALIVE_MS`, default 240s, `0` disables), and a session that still expires surfaces as a clear "voice session expired" message instead of the raw provider error.

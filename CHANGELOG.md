...
- **New Relay pairing uses each Dashboard origin as the network ingress.** Dashboard, CLI, and TUI QR flows advertise the same-origin Relay transport for LAN, Tailscale, and public HTTPS routes; direct `:8767` remains explicit legacy compatibility while `:8642` stays an optional API fallback.

...
- **Remote route probes follow the Hermes health contract.** Android uses `GET /health` instead of misclassifying the API server's `405 Method Not Allowed` response to `HEAD` as an unreachable Tailscale route, and Manage distinguishes host-side Nous provider authentication from Dashboard sign-in.

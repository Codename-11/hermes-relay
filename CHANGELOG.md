...
- **Voice: long requests get backgrounded instead of holding the floor.** Standard (non-realtime) voice now tells the model you're waiting in a live voice session, so clearly-long asks (builds, research, multi-step tool work) are delegated as background tasks with a spoken "started it in the background" acknowledgement, while quick questions keep answering directly.

...
- **Android chat and Voice stay precisely bottom-pinned through replies and keyboard changes.** The active tail keeps its stable live renderer until another row takes ownership, its timestamp space is reserved from the first frame, and bottom-following transcripts settle to the exact list boundary after replies and keyboard animations instead of leaving a small hidden remainder.

...
- **Agent sheet — collapsible Profile / Personality / Model pickers.** The three lists in the agent sheet are now space-saving dropdowns: each collapses to a header showing the current selection (e.g. "Personality — Catgirl") and expands on tap to choose, instead of rendering every option at once. The rich rows (SOUL/skills badges, provider-grouped models) are unchanged — they just live behind the header now.

- **Switch Hermes agent profiles from chat.** The Profile picker in the agent sheet now lists the host's actual agent profiles (loaded from the dashboard `/api/profiles`, like Manage and the desktop) and switching one switches the *whole agent* — its model, SOUL, personality, and skills. Verified against upstream `tui_gateway`: a profile is a full agent context, sessions are **profile-bound** (the agent is built at `session.create` from the session's profile and a live session never adopts a new one), and the desktop passes `profile` on `session.create` / `session.resume`. So the gateway client now carries the selected profile on session create/resume, and switching starts a fresh session under the new profile — the agent actually becomes it, instead of the header reading "Gary" while the running agent still answered "Victor". (SSE turns already carry the profile per-request as `profileName`.) The session drawer is now **per-profile** on the gateway too: it lists (and loads each tapped session's transcript) via the dashboard `GET /api/sessions?profile=<name>` / `…/messages?profile=<name>` surface — which opens *that profile's own* `state.db` directly, exactly how the official desktop sidebar scopes — instead of the api_server's shared DB. (The gateway `session.list` RPC can't substitute: it reads one process-global DB pinned to the launch profile, so over a single socket it never re-scopes.) Switching is also smoother: it no longer double-resets the thread (the profile-context switch alone cancels any in-flight turn and lands on the new agent's last session or a fresh chat), the empty chat now reads "Chat with <Agent>" + the agent's description so a switch is legible in the thread itself, a new chat keeps its drawer row (stamped with your first message) while its turn is in flight, and the drawer shows a spinner instead of flashing "No sessions yet" while a profile's list loads. Cold start now hydrates the host's profile list **eagerly** when a connection goes active (not lazily on first agent-sheet open), so a persisted profile selection resolves immediately — the header shows the right agent from launch instead of flashing the default and then snapping to your profile (and re-scoping the chat) only once you opened the picker.
...
- **Docs site: Android getting-started reworked + Google Play front-and-center.** The "Get the Android app" page was a flat wall of setup commands; it's now three clear steps (install → point at Hermes → connect) with the official **Get it on Google Play** badge as the primary install action, and a matching badge in the home hero. All the technical depth (sideload install + SHA256/cert verification, first-time server setup, dashboard auth, build-from-source) is preserved but tucked into collapsible `details` blocks and OS code-group tabs so new/non-technical users aren't scared off. The home-hero phone mockup's input bar was updated to the redesigned chatbar (no slash button, single morphing Send/Voice/Stop trailing slot, waveform voice glyph), and the Chat guide now documents steering, edit-and-resend, the context meter, subagent lanes, interactive ask cards, and turn-complete notifications. Product-name copy normalized to **Hermes-Relay** (hyphenated) across docs and user-facing app strings — "Relay" alone now only ever means the relay server/plugin component.
...
- **A single failed health probe no longer means 30 seconds of "offline".** The API health check ran on a fixed 30-second timer, so one transient miss at launch (probing a route the resolver was still switching, Wi-Fi not yet settled) left the app showing offline until the next tick — measured on camera as launch times swinging from ~6s to ~28s against the same healthy server. An unreachable verdict now triggers a short burst of quick re-probes (recovering within seconds), while a genuinely down server fails one burst and returns to the normal cadence. If the startup gate ever has to give up on its 12-second backstop, it now records exactly which readiness conditions were unmet in Settings → Diagnostics.
...
- **In-app "What's New" renders cleanly.** The dialog pasted the raw `whats_new.txt` into one text block, so bullets showed as literal `*` and the section headers had no emphasis. It now parses the format — version subtitle, bold section headers, real `•` bullets with hanging indent and spacing.

...
| Dashboard tab and plugin API | Dashboard plugin manifest plus plugin API routes under the Hermes dashboard plugin mount | `plugin/dashboard/manifest.json`, `plugin/dashboard/plugin_api.py` | Aligned wrapper | Dashboard routes may proxy Relay state and the explicit same-origin client allowlist, but discovery and mounting stay upstream-native. HTTP inherits Dashboard auth and carries Relay authority separately; WebSockets reuse the current upstream Dashboard ticket and request guards before Relay auth. |
...
| Dashboard same-origin Relay ingress | `plugin/dashboard/plugin_api.py`, `plugin/relay/server.py`, Android Relay clients and pairing parser | Users should not need to configure a second public port for Relay-owned capabilities. Upstream mounts plugin `APIRouter` HTTP and WebSocket routes, while Relay remains independently supervised on loopback. | Explicit method/path allowlist only; Dashboard auth and a separate Relay session header for HTTP; current upstream `_ws_request_is_allowed` plus `_ws_auth_ok` before Relay first-frame auth; fail closed when those helpers are absent; direct Relay remains compatibility fallback. | Thin or remove after upstream publishes a stable public plugin-WebSocket admission helper and/or a native remote-client transport with equivalent grants. |
...
- The same-origin Relay WebSocket currently feature-detects private upstream
  `hermes_cli.web_server._ws_request_is_allowed` and `_ws_auth_ok`, matching the
  bundled Dashboard plugin pattern. Missing helpers fail closed and clients may
  use an independently configured direct Relay route. Replace these imports when
  upstream publishes a public plugin-WebSocket admission helper.

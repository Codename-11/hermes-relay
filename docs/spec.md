...
- **Explicit static-avatar migration** — choosing a phone image and Relay-host `GET /api/profiles/{name}/avatar` import remain device-local fallbacks. Android uploads one only after the user taps **Upload device icon to Hermes**, using `profiles.set_asset`; clearing the shared Hermes avatar does not erase the fallback. PNG/JPEG/WebP magic, declared MIME/size, decoded base64, and the exact 2,000,000-byte upstream limit are checked client-side. Animated pet packs and Sphere skins never enter `ui_meta` or profile assets.

...
- **Agent sheet — Profile section (v0.6.0)** — upstream Hermes profiles auto-discovered by the relay at `~/.hermes/profiles/*/`. Selecting one overlays `model` + `SOUL.md` (as `system_message`) on subsequent chat requests. Ephemeral — clears on Connection switch and app restart. Hidden when the server advertises no profiles. See `docs/decisions.md` §21.
...
- **Connections** (v0.6.0) — lists every paired Hermes server with a per-card status chip. Actions: rename (inline), re-pair (reuses `ConnectionWizard` with `connectionId` nav arg), revoke, remove. Add-connection button launches the standard QR flow. Settings treats a paired + briefly-disconnected connection as **Connecting** (amber) instead of **Disconnected** (red) to avoid scare-red during relay restarts. See `docs/decisions.md` §19.
...
- **Voice** — interaction mode (tap / hold / continuous), silence threshold slider, Auto-TTS toggle, realtime provider info read from `/voice/realtime/config`, fallback STT/TTS provider info read from `/voice/config`, language picker, Test Voice button
...
Realtime voice conversation via relay-hosted voice endpoints. Chat still goes
directly to the API server; voice adds a modality on top, not a separate chat
channel. The preferred speech-output path is now `/voice/realtime/*`, with the
basic `/voice/transcribe` and `/voice/synthesize` endpoints retained as
fallback utilities.
...
- `POST /voice/synthesize` — JSON `{text}` → `audio/mpeg` file. Wraps `tools.tts_tool.text_to_speech_tool`; used as the basic fallback when realtime provider playback is unavailable.
- `GET /voice/config` — provider availability + current settings from `tts:` / `stt:` in `~/.hermes/config.yaml`.
- `GET/PATCH /voice/realtime/config`, `POST /voice/realtime/session`, and `GET /voice/realtime/{session_id}` — relay-mediated realtime provider sessions. Android sends PCM input events and receives mono PCM provider deltas for direct `AudioTrack` playback.
- Realtime provider defaults are relay-owned under `realtime_voice:` in `~/.hermes-relay/config.yaml` (or `RELAY_REALTIME_VOICE_CONFIG`), then overridden by `RELAY_REALTIME_VOICE_*` env vars for temporary tests. Authenticated operator clients may patch safe defaults (`enabled`, `provider`, `model`, `voice`, `sample_rate`) through the relay. Provider secrets and local auth paths stay server-side.
...
- `RealtimePcmPlayer` streams `/voice/realtime/*` provider PCM deltas directly to `AudioTrack`.
- `VoiceViewModel` state machine (`Idle / Listening / Transcribing / Thinking / Speaking / Error`). Assistant text is sanitized (markdown / tool-annotations / URLs / emoji-set stripped) on each delta before a coalescing chunker (`MIN_COALESCE_LEN=40`, `MAX_BUFFER_LEN=400` secondary-break escape, 800 ms timer flush) emits sentence-scale chunks. The preferred queue calls `/voice/realtime/*` for PCM playback; failed realtime turns fall back to the existing `/voice/synthesize` synth/play workers. The same stream observer watches Hermes-owned `ToolCall` state and speaks bounded status lines for running tools; execution, approval, and tool results remain in the Hermes chat/relay loop.
...
- Voice Settings screen off the main Settings — interaction mode, silence threshold, realtime provider/model/voice/auth labels, fallback TTS/STT provider labels, Test Voice button. Test Voice uses the legacy Hermes TTS backend and is a fallback-path check; normal assistant speech prefers realtime provider PCM when available.

See `docs/decisions.md` → **Voice Mode — Architecture** for the historical baseline decisions. Current realtime voice updates the recorder to PCM/WAV and routes assistant speech through `/voice/realtime/*` before falling back to the older client chunking path.

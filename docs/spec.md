...
5. **Safety & capabilities** (`BridgeSafetySummaryCard` → `BridgeSafetySettingsScreen`) — connection-scoped grouped grants appear first: device/app info, contacts, location, clipboard read/write, media control, communications, and outbound sharing use explicit Always/Never switches; screen/UI inspection and screen/device control are Timed-only. The same screen retains the global blocklist, destructive verbs, 5–120 minute timed-access window, status overlay, and confirmation timeout. The summary countdown describes timed screen authority, not the master switch or permanent read grants.

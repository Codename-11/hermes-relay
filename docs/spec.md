...
- **Appearance customization** — each authored preset has an expandable quick editor directly below mode. Accent and Soft/Balanced/Sharp shape changes apply immediately, persist locally in DataStore, and update both the live preview and real app surfaces. **Custom** opens a saved-preset workshop with a real chat preview, editable Background/Surface/Accent/Text roles, fixed Light or Dark ownership, saved shape, and bounded rename/duplicate/delete actions. Up to 20 normalized presets are stored locally; unsupported Auto/Light/Dark choices remain visibly disabled. **Reset theme** restores the Relay preset, Auto mode, authored accent, and Soft shape without deleting saved custom presets or resetting independent font, visualization, or pet choices. Derived Material on-colors preserve readable contrast. Typography and density remain in their focused Font and Font size controls.

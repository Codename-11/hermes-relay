...
- **Floating pet companion** — one root-level host preserves the selected pet across in-app navigation. The target is 56 dp with 48 dp art (48/40 dp in compact IME or short-screen layouts); only that target intercepts input and the surrounding positioning layer is click-through. Long hold then drag clamps the pet to a protected viewport, snaps it to the nearest logical start/end edge, and persists edge plus normalized vertical position so rotation, resizing, and RTL remain stable. The menu and TalkBack custom actions can move, reset, configure, pause roaming, or hide it.
- **Opt-in roaming** — off by default. Enabling it docks the pet onto Chat's dedicated 56 dp safe rail above the composer; autonomous motion is allowed only while Chat is visible, Hermes is idle, and that rail is registered. A manual drag or TalkBack move-up/down action pauses roaming before applying the free-form placement. Agent activity, transcript scrolling, menu interaction, app backgrounding, startup, voice and clean/ambient presentation, disabled animations, Android animator scale 0, or TalkBack touch exploration also pause motion. Outside Chat the pet stays docked at its saved edge.

...
2. **Agent access cockpit** (`BridgeAgentAccessCard`) — always visible directly below Master. It shows the current preset/custom posture, permanent-grant count, timed screen state/countdown, and effective unattended state. A first-use **Set up access** sheet offers Read only, Read + confirmed actions, or Custom; presets atomically replace permanent grants, while Custom opens the complete grouped editor. Temporary inspection/control uses a separate sheet with explicit 5 min / 30 min / 2 hr idle duration, prerequisite truth, and End now.
3. **Android access disclosure** (`BridgeAndroidAccessSummaryCard` + `BridgeSelectedAndroidAccessCard`) — summarizes readiness only for capabilities the user selected. Missing permissions for disabled capabilities never nag. Expanding preserves the complete existing `BridgePermissionChecklist` with every status, Settings link, and Test action (Core bridge / Notification companion / Voice & camera / Sideload features). The selected-access card leads with the still-missing requirements before the full matrix. Reads the same `AppPermissionStatusProbe` snapshot as Settings -> Permissions and re-probes on resume.
4. **Advanced divider** — visual separator between ordinary access/readiness and unattended/safety power controls.
5. **Unattended Access card** (`UnattendedAccessRow`, sideload-only) — opt-in toggle gated on both Master and an active timed Screen control grant. First-enable shows the warning dialog covering the security model + credential-lock limitation + how to disable. Expiry, End now, or Master-off clears the unattended preference, so a later screen grant cannot silently revive it. The cockpit mirrors effective state above the fold; this full card retains warning/keyguard detail.

...
- **Pet terrain diagnostics** — debug builds expose a default-off Developer Options toggle that paints the protected viewport, measured perches, derived walk rails, narrow-bubble touchdown points, active rail, expanded collision regions, current footprint, most recently planned route, and behavior gate over the live app. The Canvas is pointer-transparent and never changes route selection or release behavior; locking Developer Options clears it.

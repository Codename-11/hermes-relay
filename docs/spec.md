...
- **Voice** — interaction mode (tap / hold / continuous), silence threshold slider, Auto-TTS toggle, default voice-output renderer settings from `/voice/output/config`, realtime-agent lab info from `/voice/realtime/config`, fallback STT/TTS provider info from `/voice/config`, language picker, Test Voice button
...
first-class streaming TTS renderer. The realtime-agent path remains available
at `/voice/realtime/*` for provider-agent experiments.
...
- `GET /voice/output/providers/{provider_id}/options` and `GET /voice/realtime/providers/{provider_id}/options` — provider-specific option refresh before saving. Android calls these when a provider is selected so dynamic account-backed choices can be fetched by the relay without exposing provider secrets. xAI refreshes built-in/paginated custom voices when API/OAuth auth is available; ElevenLabs refreshes voices/models/languages with its API key; OpenAI uses static documented voice choices. Responses include `schema_version`, grouped voice metadata, recommended/custom flags, and model/voice compatibility hints when known. Unknown or unauthenticated discovery falls back to static provider metadata plus manual entry.
- `POST /voice/output/providers/{provider_id}/validate` and `POST /voice/realtime/providers/{provider_id}/validate` — pre-save validation for provider/model/voice/sample-rate selections. Unknown manual IDs return warnings; explicit incompatibilities return blocking errors.
...
- `RealtimePcmPlayer` streams `/voice/output/*` and `/voice/realtime/*` PCM deltas directly to `AudioTrack`.
...
- Voice Settings screen off the main Settings — interaction mode, silence threshold, editable voice-output provider/model/voice/language/latency/fallback settings, editable experimental realtime-agent defaults behind dev options, fallback TTS/STT provider labels, active profile/scope labels, and Test Voice button. Dropdowns come from relay-advertised provider metadata, refresh through provider-specific options routes when the selected provider changes, become searchable/grouped for large voice catalogs, and validate compatibility before saving, with advanced manual entry for raw provider/model/voice IDs. Voice routes receive the selected Hermes profile; the relay reports whether values came from profile config, relay config, or global fallback. Test Voice uses `/voice/output/*` first so it follows active profile voice-output defaults, with legacy Hermes TTS as a fallback; normal assistant speech uses the same streaming renderer PCM path when available.

See `docs/decisions.md` → **Voice Mode — Architecture** for the historical baseline decisions. Current voice mode records PCM/WAV for STT, routes assistant speech through `/voice/output/*`, and keeps `/voice/realtime/*` as a provider-agent lab path.

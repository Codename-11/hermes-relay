...
- **Floating pet companion** — one root-level overlay host preserves the selected pet across in-app navigation without reserving transcript or composer layout space. The target is 56 dp with 48 dp art (48/40 dp in compact IME or short-screen layouts); only that target intercepts input and the surrounding positioning layer is click-through. Long hold then drag clamps the pet to a protected viewport, avoids registered interactive surfaces, snaps it to the nearest logical start/end edge, and persists edge plus normalized vertical position so rotation, resizing, and RTL remain stable. The menu and TalkBack custom actions can move, reset, configure, pause roaming, or hide it.
- **Opt-in roaming** — off by default. Chat measures the existing composer at runtime and exposes its top edge as a Desktop-style walkable perch; no spacer is inserted. Enabling roaming docks the pet onto that perch, where directional Petdex run rows track travel without covering composer controls. A long-hold drag temporarily yields the roam loop; dropping off the perch uses the Petdex `jumping` row to settle back before directional travel resumes, without disabling roaming. TalkBack move-up/down intentionally pauses roaming before applying a free-form placement. Agent activity, transcript scrolling, menu interaction, app backgrounding, startup, voice and clean/ambient presentation, disabled animations, Android animator scale 0, or TalkBack touch exploration also pause motion. Outside Chat the pet stays docked at its saved edge.

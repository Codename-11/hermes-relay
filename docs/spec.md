...
- **Floating pet companion** — one root-level overlay host preserves the selected pet across in-app navigation without reserving content or control-bar layout space. The target is 56 dp with 48 dp art (48/40 dp in compact IME or short-screen layouts); only that target intercepts input and the surrounding positioning layer is click-through. Long hold then drag clamps the pet to a protected viewport, avoids registered interactive surfaces, snaps it to the nearest logical start/end edge, and persists edge plus normalized vertical position so rotation, resizing, and RTL remain stable. The menu and TalkBack custom actions can move, reset, configure, pause roaming, or hide it.
- **Opt-in, route-aware roaming** — off by default. Screens deliberately register curated UI ledges and obstacles using their live measured bounds; Android does not scan arbitrary elements or infer safe geometry from accessibility semantics. Chat registers the top edge of its existing composer and a single outer-edge landing point on the newest visible settled assistant bubble, while Terminal registers its extra-keys toolbar, with no inserted spacer. Rails include visual clearance above content, and the pet walks to the bubble's screen-side edge before jumping vertically, so its footprint and transfer path do not cross message text or controls. The bubble is a brief landing/wave point rather than a horizontal patrol rail; normal edge-to-edge travel remains above the composer. Chat's visible scroll-to-bottom button and Terminal's visible jump-to-latest control trim or block only the rail segments they occupy. A transition between supported route perches, off-perch drop, ambient hop, or hop between ledges uses the Petdex jump/fall clips; horizontal movement along a ledge uses directional walking clips, and an idle patrol may wave. TalkBack move-up/down intentionally pauses roaming before applying a free-form placement. Agent activity, transcript scrolling, drag/menu interaction, app backgrounding, startup, voice and clean/ambient presentation, disabled animations, Android animator scale 0, or TalkBack touch exploration pause motion without re-docking the overlay. Routes without a registered ledge keep the pet docked at its saved edge.

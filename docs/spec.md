...
- **Appearance** — theme (auto/light/dark), dynamic colors toggle, independent **Background visualization** (Off/Sphere with Sphere skins) and **Floating pet** (None/installed pet) controls. Pets can be installed from the searchable Petdex catalog or imported as a custom Relay `.zip`/single image. Petdex browsing prefers its v2 manifest with v1 fallback; atlas downloads are user-initiated, exact-host and size constrained, attribution-preserving, validated, and installed atomically for offline use. Existing combined selections migrate once: `sphere` becomes no companion, while a pet id remains the selected companion. Profile identity is unaffected.

...
- **Hermes Management** — the existing Dashboard-backed management route uses a grouped, summary-first hierarchy instead of a flat replica of the Desktop dashboard. It identifies the managed server first, warns that server-scoped changes can affect every connected client, and groups destinations as **Agents** (profiles and models), **Capabilities** (skills, MCP, and catalog), **Operations** (automations), and **Server & providers** (custom endpoints, keys, and advanced config). Server topology and component diagnostics stay collapsed under **Server details** until requested. Detail headers label the effective scope (`Profile`, `Server`, or `Secret`) while retaining the standard upstream Dashboard endpoints and authentication contract; Android does not create a parallel configuration store.

...
- **Separated shared and phone avatar controls** — **Shared across Hermes** directly selects or removes the upstream `profiles.set_asset` avatar without changing local presentation. **This phone only** is a persisted per-connection/profile override populated from a phone image or Relay-host `GET /api/profiles/{name}/avatar`; selecting an image enables the override, while disabling it immediately returns to the shared avatar. Phone-local PNG/JPEG/WebP/GIF bytes are magic-checked and capped at 8 MB; Coil renders animated GIF/WebP consistently anywhere the profile icon appears. Shared bytes retain the exact upstream PNG/JPEG/WebP and 2,000,000-byte contract. Pet sheets and Sphere skins never enter `ui_meta` or profile assets.

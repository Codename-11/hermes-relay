...
- **Hermes Management** — preserves the compact section hub, then gives Profiles a focused mobile inspector with an explicit connection target, horizontal profile selector, upstream-backed model and skill summaries, direct SOUL access, grouped profile actions, and an explained Server actions section. Gateway-running status is distinct from the sticky active-profile mutation, which changes future CLI/gateway defaults without retargeting connected clients; unsupported SOUL-presence, Memory, or session-count summaries are not inferred.

...
- **Agent profile defaults** — profile-scoped editor for future-session defaults on the selected Hermes installation. The screen freezes the target connection and canonical profile for its lifetime, stages model, reasoning, speed, personality, and approval changes for review, and reads/writes through the standard authenticated Dashboard API with an explicit `profile` parameter. Voice defaults carry the same profile target into the standard Voice editor. Connection changes invalidate the draft; profile-default controls never reuse live-session setters or Relay-only configuration routes. Unsupported standard fields remain read-only instead of falling through to server-global or Relay-scoped writes.

...
- **Hermes Management** — the existing Dashboard-backed hub remains the summary-first entry point rather than replicating the Desktop dashboard. It identifies the managed server, warns when changes affect every connected client, and groups destinations as **Agents**, **Capabilities**, **Operations**, and **Server & providers**, with topology and diagnostics collapsed under **Server details**. Cleaned detail views organize controls into **Integrations**, **Automations**, and **Server configuration** families; every editable value states whether it applies to the selected `Profile` or `Server-wide`. Edits are reviewed before save, stored secrets stay hidden, and destructive actions are separated from routine controls and require confirmation. Reads and writes continue through the standard authenticated upstream Dashboard contract, without a parallel Android configuration store, Relay-only dependency, or fork patch.
